...
A score of 0 indicates that the text is very objective and factual.
A score of 1 indicates that the text is very subjective and opinionated.
"""

import plotly.graph_objects as go
import plotly.express as px
# Trend from Date (Line Chart)
df['date'] = pd.to_datetime(df['date'])
daily_tweet_count = df.groupby('date').size().reset_index(name='tweet_count')
fig1 = px.line(daily_tweet_count, x='date', y='tweet_count', title='Trend of Tweet Count over Time')

# Sentiment by Country (Choropleth Map)
country_sentiment = df.groupby('country')['polarity'].mean().reset_index(name='avg_sentiment')
...
fig3 = go.Figure(data=go.Heatmap(z=corr.values,
                                  x=corr.index.values,
                                  y=corr.columns.values))

# Tweet Count by Day of Week (Bar Chart)
day_of_week_tweet_count = df['day_of_week'].value_counts().reset_index()
...
# Add a new column 'frame' with sequential values
daily_tweet_count['frame'] = range(len(daily_tweet_count))

# Create an animated line chart
fig5 = px.line(daily_tweet_count, x='date', y='tweet_count', title='Trend of Tweet Count over Time', animation_frame='frame')
